...
@section('title','Tambah Data Proses Diagnosa')
...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('admin.proses.store') }}" method="POST">
                @csrf
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="kode_gejala" class="form-control" required>
                        ...
                        @foreach($gejalas as $gejala)
                            <option value="{{ $gejala->kode_gejala }}" {{ old('kode_gejala') == $gejala->kode_gejala ? 'selected' : '' }}>
                ...
                <div class="form-group mb-3">
                    <label for="nilai_cf">Nilai Certainty Factor (0.0 - 1.0)</label>
                    <input type="number" step="0.01" min="0" max="1" name="nilai_cf" class="form-control" placeholder="Contoh: 0.8" value="{{ old('nilai_cf') }}" required>
                    @error('nilai_cf')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
